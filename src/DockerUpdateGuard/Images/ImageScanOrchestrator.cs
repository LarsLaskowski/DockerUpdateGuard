...
public class ImageScanOrchestrator : IImageScanOrchestrator
{
                                                            ...
                                                            CancellationToken cancellationToken)
    {
        ...
        switch (runtimeDescriptor.Kind)
        {
            case DerivedBaseRuntimeKind.DotNet:
                {
                    await CreateDotNetDerivedBaseRuntimeFindingAsync(scanRun,
                                                                     observedImage,
                                                                     subjectImageVersion,
                                                                     runtimeDescriptor,
            ...
            case DerivedBaseRuntimeKind.Nginx:
                {
                    await CreateNginxDerivedBaseRuntimeFindingAsync(scanRun,
                                                                    observedImage,
                                                                    subjectImageVersion,
                                                                    runtimeDescriptor,
                                                                    runtimeDescriptor.RuntimeVersion,
    ...
    /// <summary>
    /// Create a derived base-runtime finding for an observed image running an outdated .NET runtime
    /// </summary>
    /// <param name="scanRun">Scan run</param>
    /// <param name="observedImage">Observed image</param>
    /// <param name="subjectImageVersion">Subject image version</param>
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Task</returns>
    private async Task CreateDotNetDerivedBaseRuntimeFindingAsync(ScanRun scanRun,
                                                                  ObservedImage observedImage,
                                                                  ImageVersion subjectImageVersion,
                                                                  ...
                                                                  CancellationToken cancellationToken)
    {
        var channelReleaseResult = await _dotNetReleaseMetadataService.GetChannelReleaseAsync(channelVersion, cancellationToken)
                                                                      .ConfigureAwait(false);
        ...
        if (IsPatchBehind(runtimeVersion, channelReleaseResult.Data.LatestRuntimeVersion) == false)
        {
            return;
        }
    ...
    /// <summary>
    /// Create a derived base-runtime finding for an observed image running an outdated NGINX runtime
    /// </summary>
    /// <param name="scanRun">Scan run</param>
    /// <param name="observedImage">Observed image</param>
    /// <param name="subjectImageVersion">Subject image version</param>
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Task</returns>
    private async Task CreateNginxDerivedBaseRuntimeFindingAsync(ScanRun scanRun,
                                                                 ObservedImage observedImage,
                                                                 ImageVersion subjectImageVersion,
                                                                 DerivedBaseRuntimeDescriptor runtimeDescriptor,
                                                                 Version runtimeVersion,
                                                                 ...
                                                                 CancellationToken cancellationToken)
    {
        var channelReleaseResult = await _nginxReleaseMetadataService.GetChannelReleaseAsync(channelVersion, cancellationToken)
                                                                     .ConfigureAwait(false);
        ...
        if (IsPatchBehind(runtimeVersion, channelReleaseResult.Data.LatestVersion) == false)
        {
            return;
        }
    ...
    /// <summary>
    /// Persist the resolved base images of an observed image and report the resulting scan status
    /// </summary>
    /// <param name="scanRun">Owning scan run</param>
    /// <param name="observedImage">Observed image</param>
    /// <param name="baseImagesResult">Base image resolution result</param>
    /// <param name="currentStatus">Scan status before the base images were processed</param>
    /// <param name="statusMessages">Collected status messages</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Resulting scan status and number of resolved base images</returns>
    private async Task<(ScanRunStatus Status, int ResolvedBaseImageCount)> ApplyBaseImageResolutionAsync(ScanRun scanRun,
                                                                                                         ObservedImage observedImage,
                                                                                                         ExternalOperationResult<IReadOnlyList<BaseImageDescriptor>> baseImagesResult,
                                                                                                         ScanRunStatus currentStatus,
                                                                                                         List<string> statusMessages,
                                                                                                         CancellationToken cancellationToken)
    {
        if (baseImagesResult.Status != ExternalOperationStatus.Succeeded || baseImagesResult.Data is null)
        {
            var unavailableStatus = currentStatus;

            if (unavailableStatus != ScanRunStatus.Failed)
            {
                unavailableStatus = baseImagesResult.Status == ExternalOperationStatus.Failed ? ScanRunStatus.Failed : ScanRunStatus.Partial;
            }
        ...
        {
            var baseImageFailureMessage = await ProcessBaseImageAsync(scanRun,
                                                                      observedImage,
                                                                      baseImage,
                                                                      cancellationToken).ConfigureAwait(false);

            if (baseImageFailureMessage is not null)
            {
                if (resolvedStatus != ScanRunStatus.Failed)
                {
                    resolvedStatus = ScanRunStatus.Partial;
                }

                statusMessages.Add(baseImageFailureMessage);
            }
        }
        ...
        return (resolvedStatus, baseImagesResult.Data.Count);
    }

    /// <summary>
    /// Persist a resolved base image and its relationship to the observed image
    /// </summary>
    /// <param name="scanRun">Owning scan run</param>
    /// <param name="observedImage">Observed image</param>
    /// <param name="baseImage">Resolved base image descriptor</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Failure message when the base image metadata could not be refreshed, otherwise null</returns>
    private async Task<string?> ProcessBaseImageAsync(ScanRun scanRun,
                                                      ObservedImage observedImage,
                                                      BaseImageDescriptor baseImage,
                                                      CancellationToken cancellationToken)
    {
        ...
        {
            failureMessage = baseTagResult.Message ?? $"Unable to refresh exact base image '{baseImageReference.FullReference}'";
                                ...
                                CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                var baseImageOutcome = await ApplyBaseImageResolutionAsync(scanRun,
                                                                           observedImage,
                                                                           baseImagesResult,
                                                                           finalStatus,
                                                                           statusMessages,
                ...
                finalStatus = baseImageOutcome.Status;
                resolvedBaseImageCount = baseImageOutcome.ResolvedBaseImageCount;
    ...
    #endregion // IImageScanOrchestrator
}