...
- Review any newly added or changed code (since the previous review) against the same criteria `review-pr` uses: correctness (bugs, edge cases, null handling, concurrency, async/`.ConfigureAwait(false)`, EF Core navigation assumptions), convention adherence (`CLAUDE.md` and `.github/instructions/csharp.instructions.md`), scope/size, tests (`src\Tests`, `{Class}{Scenario}{ExpectedResult}` naming, MSTest assertions with messages), and clarity.
